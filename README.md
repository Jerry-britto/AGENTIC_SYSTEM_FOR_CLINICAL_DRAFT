...
An advanced clinician assistant powered by **LangGraph** and **Groq (Llama 3.3 70B)** designed to ingest, parse, reconcile, and compile unstructured patient medical records into structured, safe, and professional clinical discharge summaries.
...
flowchart TD
    ...
    ReaderNode["Reader Node (Llama 3.3 70B)"]
...
3. **Line-by-Line Medication Reconciliation**: Automatically cross-references admission and discharge medication lists against the hospital course narrative. Flags any medication changes (additions, discontinuations, or dosage updates) lacking a documented reason as `HIGH WARNING`.
4. **Clinical Safety Safeguards**:
...
5. **Observability Trace**: Automatically captures reasoning steps, warnings, and plan checklists during execution and saves the trace to `logs/agent_trace.md`.
...
* 📂 **`clinical_agent` Module**
  ...
  * 📄 [config.py](file:///home/jerrybritto/demo/AI/clinical_draft_agent/clinical_agent/config.py): API key validator and model assignments (Groq's `llama-3.3-70b-versatile` and `llama-3.1-8b-instant`).
...
### Generated Outputs
Upon a successful execution run, the following files will be created/updated:
* 📝 **`outputs/patient_records-discharged_summary.md`**: Professional Markdown clinician discharge summary containing safety/escalation banners.
* 💾 **`outputs/patient_records-discharged_summary.json`**: Machine-readable JSON draft data formatted for database insertion.
* 🔍 **`logs/agent_trace.md`**: Decisional trace logging explaining every step the planner reasoning loop made.
